...
from app.models import Base, HumanEvent, Session
...
async def secured_app(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(main_module.settings,"auth_username","csl")
    monkeypatch.setattr(main_module.settings,"auth_password_hash",hash_password("shared-pass"))
...
async def login(client,remember=False):
    return await client.post("/api/auth/login",json={"username":"csl","password":"shared-pass","remember":remember},headers={"origin":ORIGIN})
...
async def test_auth_roles_remember_logout_and_revocation(secured_app):
    ...
    async with httpx.AsyncClient(transport=transport,base_url=ORIGIN) as first, httpx.AsyncClient(transport=transport,base_url=ORIGIN) as admin:
        ...
        remembered=await login(first,True); assert remembered.status_code==200
        ...
        assert (await first.post("/api/auth/admin/verify",json={"password":"admin-pass"})).status_code==403
        assert (await first.post("/api/auth/admin/verify",json={"password":"wrong"},headers=csrf_headers(first))).status_code==401
...
async def test_login_rate_limit_and_origin_csrf(secured_app):
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=main_module.app),base_url=ORIGIN) as client:
        no_origin=await client.post("/api/auth/login",json={"username":"csl","password":"shared-pass"}); assert no_origin.status_code==403
        for _ in range(5): assert (await client.post("/api/auth/login",json={"username":"csl","password":"bad"},headers={"origin":ORIGIN})).status_code==401
        assert (await client.post("/api/auth/login",json={"username":"csl","password":"bad"},headers={"origin":ORIGIN})).status_code==429
...
async def test_deactivation_blocks_signals_without_deleting_history(secured_app,monkeypatch):
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=main_module.app),base_url=ORIGIN) as client:
        ...
        assert (await client.post("/api/auth/admin/verify",json={"password":"admin-pass"},headers=headers)).status_code==200
