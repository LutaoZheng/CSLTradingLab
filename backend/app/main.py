...
from .models import Base, Session, Market, HumanEvent, Quote, Trade, BookEvent, RawMessage, SystemEvent
...
class EventReq(BaseModel):
    ...
    device_wall_ts_ms:float; device_perf_ts_ms:float|None=None; score_at_click:dict|None=None
...
async def state(sid:str):
    ...
    return {"session":obj(s),"markets":[obj(x) for x in ms],"events":[obj(x) for x in ev],"quotes":quotes,"score":score.get(s.event_ticker),"health":kalshi.health(sid),"recorder":recorder.health(sid),"preflight":preflight(sid)}
...
async def delete_session(sid:str,req:DeleteReq):
    async with focus_lock:
        ...
        try:
            async with maker() as db:
                deleted={}
                for model in (HumanEvent,Quote,Trade,BookEvent,RawMessage,SystemEvent):
...
async def human_event(sid:str,req:EventReq):
    async with human_event_lock:
        return await _append_human_event(sid,req)

async def _append_human_event(sid:str,req:EventReq):
    server_ns=time.time_ns(); latency=server_ns/1e6-req.device_wall_ts_ms
    async with maker() as db:
        if not await db.get(Session,sid): raise HTTPException(404)
        if await db.get(HumanEvent,req.event_id): return {"ok":True,"duplicate":True}
        s=await db.get(Session,sid)
        ...
        e=HumanEvent(id=req.event_id,event_group_id=req.event_group_id,session_id=sid,match_id=s.event_ticker,device_wall_ts_ms=req.device_wall_ts_ms,device_perf_ts_ms=req.device_perf_ts_ms,server_receive_ts_ns=server_ns,phone_to_backend_latency_ms=latency,event_type=req.event_type,team=req.team,score_at_click=req.score_at_click,kalshi_match_clock_at_click=req.kalshi_match_clock_at_click,target_event_id=req.target_event_id,detail=req.detail); db.add(e); await db.commit()
    raw_path=settings.data_dir/"raw"/f"match_{s.event_ticker}"/sid/"human_events.ndjson"
    raw_line=json.dumps({"server_receive_ts_ns":server_ns,**obj(e)},default=str,separators=(",",":"))+"\n"
    await asyncio.to_thread(_append_text,raw_path,raw_line)
    await broadcast({"type":"human_event","item":obj(e)}); return {"ok":True,"server_receive_ts_ns":server_ns,"phone_to_backend_latency_ms":latency}
...
async def session_data(sid:str,limit:int=200):
    ...
    return {"session":obj(s),"status":"ACTIVE" if s.ended_at is None else "STOPPED","duration_seconds":max(0,(duration_end-duration_start).total_seconds()) if duration_start else 0,"counts":counts,"recorder":rh,"timeline":timeline}
...
async def replay(sid:str,event_id:str,before:float=10,after:float=30):
    async with maker() as db:
        e=await db.get(HumanEvent,event_id)
        if not e or e.session_id!=sid: raise HTTPException(404)
        lo=int(e.device_wall_ts_ms*1e6-before*1e9); hi=int(e.device_wall_ts_ms*1e6+after*1e9)
    ...
    return {"event":obj(e),"quotes":[{**obj(q),"relative_ms":(q.local_recv_ts_ns-e.device_wall_ts_ms*1e6)/1e6} for q in qs],"sampling":"exact event-driven updates; no interpolation"}
...
def _build_session_export(target:Path,sid:str,session_record:dict):
    ...
    try:
        ...
        session_json={**session_record,"session_id":sid,"mode":session_record.get("session_type"),"status":status,"match_title":f'{session_record["home_team"]} vs {session_record["away_team"]}',"exported_at":exported_at,"market_tickers":[m["ticker"] for m in markets],"market_families":families,"market_family_counts":{name:sum(family(m)==name for m in markets) for name in families}}
        ...
        with zipfile.ZipFile(target,"w",zipfile.ZIP_DEFLATED,allowZip64=True) as z:
            ...
            human_headers=["event_id","event_group_id","event_type","team","device_wall_timestamp","device_wall_timestamp_ms","performance_timestamp_ms","server_receive_timestamp","server_receive_timestamp_ns","phone_to_backend_latency_ms","match_id","score_at_click","kalshi_match_clock_at_click","target_event_id","reason","detail"]
            def human_transform(x):
                detail=json.loads(x["detail"] or "{}") if isinstance(x.get("detail"),str) else (x.get("detail") or {})
                return {**x,"event_id":x.pop("id"),"device_wall_timestamp":_iso_ms(x["device_wall_ts_ms"]),"device_wall_timestamp_ms":x.pop("device_wall_ts_ms"),"performance_timestamp_ms":x.pop("device_perf_ts_ms"),"server_receive_timestamp":_iso_ns(x["server_receive_ts_ns"]),"server_receive_timestamp_ns":x.pop("server_receive_ts_ns"),"reason":detail.get("reason","")}
